...
import { Loader2, Sparkles, ArrowRight, Lock, Mail, User } from 'lucide-react';
      ...
      <div className="w-full max-w-sm flex flex-col gap-2 sm:gap-2.5">
        ...
        <form onSubmit={signupHandler} className="flex flex-col gap-2">
          <div className="space-y-1">
            <Label htmlFor="username" className="text-xs font-semibold text-rose-950/70 dark:text-rose-200/70">
              Username
            </Label>
            <div className="relative">
              <User className="w-4 h-4 text-rose-400 dark:text-rose-500 absolute left-3.5 top-1/2 -translate-y-1/2 pointer-events-none" />
              <Input
                type="text"
                id="username"
                name="username"
                required
                placeholder="cool_vibes"
                value={input.username}
                onChange={changeEventHandler}
                disabled={isBusy}
                className="h-9 sm:h-9.5 pl-10 text-sm rounded-xl bg-card/80 border-rose-200/70 dark:border-rose-900/50 text-foreground placeholder:text-muted-foreground focus-visible:ring-rose-500/30 focus-visible:border-rose-500 transition shadow-sm"
              />
            </div>
          </div>
          ...
          <Button
            type="submit"
            disabled={isBusy || !input.username || !input.email || !input.password}
            className="bg-gradient-to-r from-rose-600 via-pink-600 to-rose-500 hover:from-rose-700 hover:via-pink-700 hover:to-rose-600 text-white h-9.5 sm:h-10 mt-1 rounded-full font-bold shadow-md shadow-rose-500/30 hover:shadow-lg hover:shadow-rose-500/40 transition-all active:scale-[0.99] flex items-center justify-center gap-2 text-sm"
          >
            {loading ? (
              <>
        ...
        {/* Legal disclaimer */}
        <p className="text-[10px] sm:text-[11px] leading-tight text-muted-foreground text-center mt-0.5">
        ...
        {/* Switch to Login */}
        <div className="mt-1 pt-2 border-t border-rose-200/60 dark:border-rose-900/40 text-center">
